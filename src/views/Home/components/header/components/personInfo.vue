  ...
  <div id="personInfo">
    ...
    <a-popover placement="bottomRight" class="photoPopover">
      ...
      <template slot="content">
        ...
        <p class="photoPopoverBtn" style="padding: 5px 10px; margin-bottom: 5px;" @click="showModal">
          我发布的
        </p>
        <a-divider />
        <p class="photoPopoverBtn" style="padding: 5px 10px; margin-top: 5px;" @click="logout">
          注销
        </p>
      </template>
      ...
      <a-modal v-model="modalVisible"
        ...
        <a-table :columns="columns" :data-source="data" :pagination="{pageSize: 6}">
          <span slot="submissionType" slot-scope="tags">
            <a-tag
              :key="tags"
              :color="tags === 'find' ? 'geekblue' : 'green'">
              {{ tags == 'find' ? '寻物启事' : '失物招领' }}
            </a-tag>
          </span>
...
export default {
  ...
  methods: {
    ...mapActions(['onLogout']),
    createProject ({ key }) {
      this.$store.commit('showModal', {formType: key, createOrUpdate: 'create'});
    },
    ...
    onUpdate (item) {
      this.$store.commit('showModal', { formType: item.submissionType, createOrUpdate: 'update', itemData: item });
    },
    logout () {
